...
interface NPCAction {
  ...
  category: 'npc' // New field to distinguish action types
}

interface ActionDefinitions {
  version: string
  enabled_default_actions: string[]
  default_action_definitions: NPCAction[]
  custom_actions: NPCAction[]
  action_categories: Record<string, string[]>
  global_settings: Record<string, any>
}

interface NPCActionConfig {
  version: string
  custom_actions: NPCAction[]
  enabled_default_actions: string[]
  action_categories: Record<string, string[]>
    ...
    <div className="space-y-4 border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-gray-50 dark:bg-gray-800">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">Define configurable parameters for this NPC action</p>
        </div>
        <button
...
const NPCActionConfig: React.FC = () => {
  const [config, setConfig] = useState<NPCActionConfig | null>(null)
  const [actionDefinitions, setActionDefinitions] = useState<ActionDefinitions | null>(null)
  const [allActions, setAllActions] = useState<NPCAction[]>([])
  ...
  useEffect(() => {
    if (config && actionDefinitions) {
      // Combine all actions into one list for NPCs
      const defaultActions: NPCAction[] = actionDefinitions.default_action_definitions.map(def => ({
        ...def,
        affects_mood: false,
        creates_memory: true,
        visibility: 'public',
  ...
  const saveConfig = async () => {
    ...
    try {
      ...
      alert('NPC actions configuration saved successfully!')
    } catch (err) {
  ...
  const addAction = async () => {
    ...
    try {
      setSaving(true)
      await axios.put(`${API_BASE}/config/actions`, updatedConfig)
      resetNewAction()
      alert('NPC action added successfully!')
    } catch (err) {
  ...
  const editAction = (actionId: string) => {
    ...
    if (action) {
      setNewAction({ ...action, category: 'npc' })
  ...
  const saveEditedAction = async () => {
    ...
    try {
      ...
      resetNewAction()
      alert('NPC action updated successfully!')
    } catch (err) {
      console.error('Error saving action:', err)
      alert('Failed to save action')
    } finally {
  ...
  const removeAction = async (actionId: string) => {
    ...
    if (window.confirm(`Are you sure you want to delete the NPC action "${actionId}"?`)) {
      const updatedConfig = {
        ...config,
      ...
      try {
        setSaving(true)
        await axios.put(`${API_BASE}/config/actions`, updatedConfig)
        alert('NPC action deleted successfully!')
      } catch (err) {
  ...
  const syncFromPlayerActions = async () => {
    if (!config) return
    
    if (window.confirm('This will copy all Player actions to NPC Actions. Existing NPC actions will be kept. Continue?')) {
      try {
        setSaving(true)
        
        // Get Player actions
        const playerResponse = await axios.get(`${API_BASE}/config/player-actions`)
        const playerConfig = playerResponse.data
        
        // Convert Player actions to NPC actions
        const playerActions: NPCAction[] = playerConfig.player_actions.map((action: any) => ({
          ...action,
          category: 'npc' as const,
          action_id: action.action_id.startsWith('npc_') ? action.action_id : `npc_${action.action_id.replace('player_', '')}`
        }))
        
        // Merge with existing NPC actions (avoid duplicates)
        const existingActionIds = config.custom_actions.map(a => a.action_id)
        const newActions = playerActions.filter(action => !existingActionIds.includes(action.action_id))
        
        const updatedConfig = {
          ...config,
          custom_actions: [...config.custom_actions, ...newActions]
        }
        
        setConfig(updatedConfig)
        await axios.put(`${API_BASE}/config/actions`, updatedConfig)
        
        alert(`Synced ${newActions.length} new actions from Player Actions!`)
      } catch (err) {
        console.error('Error syncing actions:', err)
        alert('Failed to sync actions from Player Actions')
      } finally {
        setSaving(false)
      }
    }
  ...
  if (loading) {
    return <div className="loading">Loading NPC action configuration...</div>
  }
  ...
  return (
    <div className="p-6 space-y-8 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-200">NPC Actions</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Configure actions that NPCs can perform using AI decision-making</p>
        </div>
        <div className="flex space-x-3">
          <button onClick={saveConfig} disabled={saving} className="btn-primary">
            {saving ? 'Saving...' : 'Save'}
          </button>
          <button onClick={syncFromPlayerActions} disabled={saving} className="btn-secondary">
            🔄 Sync from Player Actions
          </button>
        </div>
      </div>
      ...
      <div className="glass-card p-6">
        <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">
          {editingAction ? `Editing: ${editingAction}` : 'Add New NPC Action'}
        </h3>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              ...
              <input
                type="text"
                placeholder="e.g., npc_craft_sword"
            ...
            <div className="md:col-span-2 lg:col-span-4">
              ...
              <textarea
                placeholder="Describe what this NPC action does..."
          ...
          <div className="flex space-x-3">
            <button 
              ...
              disabled={saving}
            >
              {saving ? 'Saving...' : (editingAction ? 'Save Changes' : 'Add NPC Action')}
            </button>
      ...
      {/* All NPC Actions List */}
      <div className="glass-card p-6">
        <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">All NPC Actions ({allActions.length})</h3>
        ...
        {allActions.length === 0 && (
          <div className="text-center py-8 text-gray-500 dark:text-gray-400">
            <p>No NPC actions configured yet.</p>
            <p className="text-sm">Add your first NPC action above to get started.</p>
          </div>
        )}
      </div>
    </div>
...
export default NPCActionConfig 