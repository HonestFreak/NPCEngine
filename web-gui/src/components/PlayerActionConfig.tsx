...
interface PlayerAction {
  ...
  category: 'player' // New field to distinguish action types
}

interface PlayerActionConfig {
  version: string
  player_actions: PlayerAction[]
  enabled_default_player_actions: string[]
    ...
    <div className="space-y-4 border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-gray-50 dark:bg-gray-800">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">Define configurable parameters for this player action</p>
        </div>
        <button
...
const PlayerActionConfig: React.FC = () => {
  const [config, setConfig] = useState<PlayerActionConfig | null>(null)
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [editingAction, setEditingAction] = useState<string | null>(null)
  const [viewingAction, setViewingAction] = useState<string | null>(null)
  const [newAction, setNewAction] = useState<PlayerAction>({
    action_id: '',
    name: '',
    description: '',
    target_type: 'none',
  ...
  const loadConfig = async () => {
    try {
      setLoading(true)
      const response = await axios.get(`${API_BASE}/config/player-actions`)
      setConfig(response.data)
    } catch (err) {
      console.error('Error loading config:', err)
      // Create default config if none exists
      const defaultConfig: PlayerActionConfig = {
        version: "1.0",
        player_actions: [],
  ...
  const saveConfig = async () => {
    ...
    try {
      setSaving(true)
      await axios.put(`${API_BASE}/config/player-actions`, config)
      alert('Player actions configuration saved successfully!')
    } catch (err) {
  ...
  const addAction = async () => {
    ...
    if (config.player_actions.some(a => a.action_id === newAction.action_id)) {
    ...
    const updatedConfig = {
      ...config,
      player_actions: [...config.player_actions, action]
    }
    ...
    try {
      setSaving(true)
      await axios.put(`${API_BASE}/config/player-actions`, updatedConfig)
      resetNewAction()
      alert('Player action added successfully!')
    } catch (err) {
  ...
  const editAction = (actionId: string) => {
    const action = config?.player_actions.find(a => a.action_id === actionId)
    if (action) {
      setNewAction({ ...action })
  ...
  const saveEditedAction = async () => {
    ...
    const updatedActions = config.player_actions.map(a => 
      a.action_id === editingAction ? action : a
    )

    const updatedConfig = {
      ...config,
      player_actions: updatedActions
    }
    ...
    try {
      ...
      resetNewAction()
      alert('Player action updated successfully!')
    } catch (err) {
      console.error('Error saving action:', err)
      alert('Failed to save action')
    } finally {
  ...
  const removeAction = async (actionId: string) => {
    ...
    if (window.confirm(`Are you sure you want to delete the player action "${actionId}"?`)) {
      const updatedConfig = {
        ...config,
        player_actions: config.player_actions.filter(a => a.action_id !== actionId)
      }
      ...
      try {
        setSaving(true)
        await axios.put(`${API_BASE}/config/player-actions`, updatedConfig)
        alert('Player action deleted successfully!')
      } catch (err) {
  ...
  const syncFromNPCActions = async () => {
    if (!config) return
    
    if (window.confirm('This will copy all NPC actions to Player Actions. Existing player actions will be kept. Continue?')) {
      try {
        setSaving(true)
        
        // Get NPC actions
        const npcResponse = await axios.get(`${API_BASE}/config/actions`)
        const npcConfig = npcResponse.data
        
        // Convert NPC actions to player actions
        const npcActions: PlayerAction[] = npcConfig.custom_actions.map((action: any) => ({
          ...action,
          category: 'player' as const,
          action_id: action.action_id.startsWith('player_') ? action.action_id : `player_${action.action_id}`
        }))
        
        // Merge with existing player actions (avoid duplicates)
        const existingActionIds = config.player_actions.map(a => a.action_id)
        const newActions = npcActions.filter(action => !existingActionIds.includes(action.action_id))
        
        const updatedConfig = {
          ...config,
          player_actions: [...config.player_actions, ...newActions]
        }
        
        setConfig(updatedConfig)
        await axios.put(`${API_BASE}/config/player-actions`, updatedConfig)
        
        alert(`Synced ${newActions.length} new actions from NPC Actions!`)
      } catch (err) {
        console.error('Error syncing actions:', err)
        alert('Failed to sync actions from NPC Actions')
      } finally {
        setSaving(false)
      }
    }
  ...
  if (loading) {
    return <div className="loading">Loading player action configuration...</div>
  }
  ...
  return (
    <div className="p-6 space-y-8 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-200">Player Actions</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Configure actions that players can perform in the game</p>
        </div>
        <div className="flex space-x-3">
          <button onClick={saveConfig} disabled={saving} className="btn-primary">
            {saving ? 'Saving...' : 'Save'}
          </button>
          <button onClick={syncFromNPCActions} disabled={saving} className="btn-secondary">
            🔄 Sync from NPC Actions
          </button>
        </div>
      </div>
      ...
      <div className="glass-card p-6">
        <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">
          {editingAction ? `Editing: ${editingAction}` : 'Add New Player Action'}
        </h3>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="md:col-span-2 lg:col-span-4">
              ...
              <textarea
                placeholder="Describe what this player action does..."
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <label className="flex items-center space-x-2 cursor-pointer">
              <input
                ...
                className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
              />
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Affects NPC Mood</span>
            </label>
            <label className="flex items-center space-x-2 cursor-pointer">
              <input
                type="checkbox"
          ...
          <div className="flex space-x-3">
            <button 
              ...
              disabled={saving}
            >
              {saving ? 'Saving...' : (editingAction ? 'Save Changes' : 'Add Player Action')}
            </button>
      ...
      {/* Player Actions List */}
      <div className="glass-card p-6">
        <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">Player Actions ({config.player_actions.length})</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {config.player_actions.map(action => (
        ...
        {config.player_actions.length === 0 && (
          <div className="text-center py-8 text-gray-500 dark:text-gray-400">
            <p>No player actions configured yet.</p>
            <p className="text-sm">Add your first player action above to get started.</p>
          </div>
        )}
      </div>
    </div>
...
export default PlayerActionConfig 